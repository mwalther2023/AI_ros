...
class Follow:
    ...
    def subscription_callback(self, data):
        ...
        for x in range(int(225/deg_inc),int(314/deg_inc)):
            avg += data.ranges[x]
        for x in range(int(260/deg_inc),int(279/deg_inc)):
            avg2 += data.ranges[x]
        ...
        # print("Avg: "+str(avg))
        if data.ranges[int(270/deg_inc)] < .45 or avg2 < .5: # (avg < .5 and avg > .4 ) or data.ranges[315] < .2 or data.ranges[230] < .2 or 
        ...
        for x in range(0,int(44/deg_inc)):
            avg += data.ranges[x]
        for x in range(int(315/deg_inc),int(359/deg_inc)):
            avg2 += data.ranges[x]
        for x in range(int(344/deg_inc), int(359/deg_inc)):
            avg3 += data.ranges[x]
        avg += avg2
